...
export const Route = createFileRoute('/dashboard/vendor-bookings')({
...
const getVendorBookings = async (userId: number) => {
    ...
    const myListings = listings.filter((l: Listing) => l.vendor_id === myVendor.id);
    ...
    const myBookingItems = bookingItems.filter((bi: BookingItem) => myListingIds.includes(bi.listing_id));
  ...
  return (
    <div className="space-y-6">
      <div>
          ...
          <p className="text-muted-foreground">Manage bookings for your properties.</p>
      </div>
                ...
                <Card key={booking.id}>
                    <CardHeader className="pb-2">
                        <div className="flex justify-between items-start">
                            <div>
                                <CardTitle className="text-base font-bold">{booking.listing_title}</CardTitle>
                                <CardDescription className="flex items-center gap-2 mt-1">
                                    <span className="flex items-center gap-1"><User className="h-3 w-3" /> {booking.customer_name}</span>
                    ...
                    <CardContent>
                        <div className="flex justify-between items-center text-sm border-t pt-4 mt-2">
                            <div className="flex flex-col">
                                <span className="text-muted-foreground text-xs uppercase font-medium">Check-in</span>
                                <span>{format(new Date(booking.start_date), 'PPP')}</span>
                            </div>
                            <div className="flex flex-col text-right">
                                <span className="text-muted-foreground text-xs uppercase font-medium">Check-out</span>

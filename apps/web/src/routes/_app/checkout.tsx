  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    if (cart.length === 0) {
        toast.error("Your cart is empty");
                navigate({ to: '/listings', search: (prev: any) => ({ ...prev, page: 1 }) });;
        return;
    }
    ...
    try {
      ...
      const bookingPayload = {
        ...
        items: cart.map(item => {
            const nights = differenceInDays(item.dateRange.to!, item.dateRange.from!);
            ...
            return {
                ...
                selected_addons: item.selectedAddons.map(sa => ({
                    ...
                    subtotal: sa.addon.price * sa.quantity * (sa.addon.price_type === 'per_night' ? nights : 1)
                }))
            };
        })
      };
     ...
     return (
         <PageWrapper>
             <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
                 ...
                 <Button onClick={() => navigate({ to: '/listings', search: (prev: any) => ({ ...prev, page: 1 }) })}>Browse Listings</Button>
             </div>
         </PageWrapper>
     );
  }
                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Verify your booking details before payment.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            {cart.map((item) => {
                                const nights = differenceInDays(item.dateRange.to!, item.dateRange.from!);
                                return (
                                    <div key={item.id} className="flex flex-col sm:flex-row gap-4 border-b last:border-0 pb-6 last:pb-0">
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-medium">${item.listing.base_price * nights}</span>
                                            </div>
                                            <p className="text-sm text-muted-foreground">{item.listing.listing_type} in Location {item.listing.location_id}</p>
                                            ...
                                            {item.selectedAddons.length > 0 && (
                                                <div className="mt-2 text-sm text-muted-foreground">
                                                    ...
                                                    {item.selectedAddons.map(a => `${a.addon.name} (x${a.quantity})`).join(', ')}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Enter your payment information. (Secure Mock)</CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="col-span-2 space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <Input id="cardNumber" placeholder="0000 0000 0000 0000" className="pl-9" required />
                                        </div>
                                    </div>

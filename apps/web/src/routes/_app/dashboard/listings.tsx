...
import { Listing, DB_KEYS } from '@/utils/mock-db';
...
function VendorListings() {
  ...
  const displayListings = myListings.length > 0 ? myListings : (allListings?.slice(0, 4) || []);
      ...
      <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
             {displayListings.map((listing) => (
                <ListingItem key={listing.id} listing={listing} onDelete={() => handleDelete(listing.id)} />
             ))}
          </div>
          {myListings.length === 0 && (
...
function ListingItem({ listing, onDelete }: { listing: Listing, onDelete: () => void }) {
    return (
        <Card>
            <div className="aspect-video w-full bg-muted relative overflow-hidden rounded-t-lg">
                <img 
                    src={listing.image_url || "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=800&q=80"} 
                    alt={listing.title}
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input id="title" name="title" placeholder="e.g. Ocean View Villa" className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input id="price" name="price" type="number" placeholder="100" className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="description" className="text-right">
                            Description
                        </Label>
                        <Textarea id="description" name="description" placeholder="Describe your listing..." className="col-span-3" required />
                    </div>

...
const generateVendors = (users: User[]): Vendor[] => {
  ...
  for (const user of vendorUsers) {
    vendors.push({
      ...
      vendor_type: faker.helpers.arrayElement(['hotel', 'bnb', 'car_rental', 'guide', 'tour_operator']),
...
const locationsData: Location[] = [
  { id: 1, name: 'Kigali', country: 'Rwanda', latitude: -1.9441, longitude: 30.0619 },
  { id: 2, name: 'Musanze', country: 'Rwanda', latitude: -1.5034, longitude: 29.6326 },
  { id: 3, name: 'Rubavu', country: 'Rwanda', latitude: -1.6763, longitude: 29.2619 },
  { id: 4, name: 'Akagera', country: 'Rwanda', latitude: -1.8214, longitude: 30.7303 },
  { id: 5, name: 'Nyungwe', country: 'Rwanda', latitude: -2.4862, longitude: 29.1171 },
];
...
const generateListings = (count: number, vendors: Vendor[]): Listing[] => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const listingType = faker.helpers.arrayElement(['hotel_room', 'bnb', 'car', 'tour', 'guide']);
...
export const initializeMockDB = () => {
  ...
  if (!localStorage.getItem(DB_KEYS.USERS)) {
    ...
    if (!localStorage.getItem(DB_KEYS.BOOKING_ITEMS)) localStorage.setItem(DB_KEYS.BOOKING_ITEMS, JSON.stringify([]));
    if (!localStorage.getItem(DB_KEYS.TESTIMONIALS)) localStorage.setItem(DB_KEYS.TESTIMONIALS, JSON.stringify([]));

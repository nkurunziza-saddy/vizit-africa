import { Addon } from "@/utils/mock-db";
...
export function AddonSelector({ addon, onSelect, initialQuantity = 0 }: AddonSelectorProps) {
        ...
        <div className="space-y-1">
          ...
          <div className="pt-1">
             <Badge variant="secondary" className="text-xs font-normal">
                ${addon.price} <span className="text-muted-foreground ml-1">/ {addon.price_type.replace('_', ' ')}</span>
             </Badge>
          </div>
        </div>
      </div>

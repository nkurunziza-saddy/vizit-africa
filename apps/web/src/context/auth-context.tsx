...
import { login as loginAction, register as registerAction } from '@/actions/auth.actions';

interface AuthContextType {
  user: User | null;
  isLoading: boolean;
  login: (email: string, password: string) => Promise<void>;
  register: (data: Omit<User, 'id' | 'created_at' | 'role'> & { role?: User['role'] }) => Promise<void>;
  logout: () => void;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const register = async (data: Omit<User, 'id' | 'created_at' | 'role'> & { role?: User['role'] }) => {
    setIsLoading(true);
    try {
